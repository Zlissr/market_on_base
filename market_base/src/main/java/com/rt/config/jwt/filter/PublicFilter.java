package com.rt.config.jwt.filter;


import com.rt.config.jwt.TokenHolder;
import com.rt.config.jwt.config.MutableHttpServletRequest;
import com.rt.config.jwt.service.JwtValidationService;
...
public class PublicFilter extends OncePerRequestFilter {
    ...
    private String getAuthUri() {
        String authUrl = ConfJsMarket.getInstance().getApp().getKeycloakOpenidUrl() + "/auth";
        authUrl += "?response_type=" + RESPONSE_TYPE_CODE;
        authUrl += "&client_id=" + clientId;
        authUrl += "&state=" + generateState(30);
        authUrl += "&scope=" + SCOPE;
        authUrl += "&redirect_uri=" + ConfJsMarket.getInstance().getApp().getKeycloakClientUrl();
