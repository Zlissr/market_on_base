...
public abstract class ConfJsObj 
{
    ...
    protected boolean getBooleanRequired(JsonNode p_xParser, String p_sName) throws RuntimeException {
        JsonNode xNode = p_xParser.path(p_sName);

        if (xNode == null) {
            throw new RuntimeException("Error in JSON configuration, no required attribute '"
                    + p_sName + ", " + getObjName() + " '" + comments + "'");
        }

        return xNode.asBoolean();
    }
