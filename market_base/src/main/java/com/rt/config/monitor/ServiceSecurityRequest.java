...
public class ServiceSecurityRequest {
    ...
    private static int ARRAY_ERROR_SIZE = ConfJsMarket.getInstance().getApp().getErrorsSize() + 1;
    private static int ERROR_INTERVAL = ConfJsMarket.getInstance().getApp().getErrorsInterval();
    private static int ERROR_SLEEP_INTERVAL = ConfJsMarket.getInstance().getApp().getErrorsSleepInterval();
    ...
    public void updateUsingConfig() {
        ARRAY_ERROR_SIZE = ConfJsMarket.getInstance().getApp().getErrorsSize();
        ERROR_INTERVAL = ConfJsMarket.getInstance().getApp().getErrorsInterval();
        ERROR_SLEEP_INTERVAL = ConfJsMarket.getInstance().getApp().getErrorsInterval();
    }
}
